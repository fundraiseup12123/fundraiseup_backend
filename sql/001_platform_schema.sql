-- Multi-tenant platform schema. Run in Supabase SQL editor after donations.sql.
...
-- Public read live campaign content
drop policy if exists "Public read live campaigns" on public.campaigns;
create policy "Public read live campaigns" on public.campaigns for select using (status = 'live');

drop policy if exists "Public read campaign content" on public.campaign_content;
create policy "Public read campaign content" on public.campaign_content for select using (
...
create policy "Public read campaign currencies" on public.campaign_currencies for select using (
  exists (select 1 from public.campaigns c where c.id = campaign_id and c.status = 'live')
);

drop policy if exists "Public read verified domains" on public.domains;
create policy "Public read verified domains" on public.domains for select using (verified_at is not null);

-- Members can read their org data
drop policy if exists "Members read own org" on public.organizations;
create policy "Members read own org" on public.organizations for select using (
  exists (
...
create policy "Members read campaigns" on public.campaigns for select using (
  exists (
    select 1 from public.organization_members m
...
create policy "Members manage campaigns" on public.campaigns for all using (
  exists (
    select 1 from public.organization_members m
...
-- Campaign-scoped public donations feed
drop policy if exists "Public read donations" on public.donations;
create policy "Public read donations" on public.donations for select using (
  campaign_id is null or exists (
    select 1 from public.campaigns c where c.id = campaign_id and c.status = 'live'
  )
);

-- Seed: set super admin by email after first signup (run manually):
-- update public.profiles set role = 'super_admin' where id = (select id from auth.users where email = 'your@email.com');
